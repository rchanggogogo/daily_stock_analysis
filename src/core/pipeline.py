...
class StockAnalysisPipeline:
    ...
    def fetch_and_save_stock_data(
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"获取实时行情失败，将使用默认名称: {e}")

            # 如果实时行情没有名称，使用映射表或默认值
            if not stock_name:
        ...
        try:
            ...
            try:
                ...
                if realtime_quote:
                    ...
                    logger.info(
                        f"[{code}] 实时行情获取失败或已禁用，将使用历史数据进行分析"
                    )
            except Exception as e:
            ...
            if not use_agent:
                ...
                if configured_skills and configured_skills != ["all"]:
                    use_agent = True
                    logger.info(
                        f"[{code}] Auto-enabled agent mode due to configured skills: {configured_skills}"
                    )
        ...
        try:
            ...
            if self.search_service.is_available:
                try:
                    news_response = self.search_service.search_stock_news(
                        stock_code=code, stock_name=stock_name, max_results=5
                    )
                    if news_response.success and news_response.results:
                        ...
                        logger.info(
                            f"{stock_name}({code}) Agent 模式: 新闻情报已保存 {len(news_response.results)} 条"
                        )
                except Exception as e:
                    logger.warning(
                        f"{stock_name}({code}) Agent 模式保存新闻情报失败: {e}"
                    )
            ...
            if result:
                try:
                    ...
                    logger.warning(f"{stock_name}({code}) 保存 Agent 分析历史失败: {e}")

            return result

        except Exception as e:
            logger.error(f"{stock_name}({code}) Agent 分析失败: {e}")
            logger.exception(f"{stock_name}({code}) Agent 详细错误信息:")
            return None
    ...
    def process_single_stock(
        ...
        logger.info(f"========== 开始处理 {stock_name}({code}) ==========")
        ...
        try:
            ...
            if not success:
                logger.warning(f"{stock_name}({code}) 数据获取失败: {error}")
                # 即使获取失败，也尝试用已有数据分析

            # Step 2: AI 分析
            if skip_analysis:
                logger.info(f"{stock_name}({code}) 跳过 AI 分析（dry-run 模式）")
                return None
            ...
            if result:
                # 使用 result 中的真实名称（可能从分析过程中更新）
                display_name = result.name or stock_name
                logger.info(
                    f"{display_name}({code}) 分析完成: {result.operation_advice}, "
                ...
                if single_stock_notify and self.notifier.is_available():
                    try:
                        ...
                        if report_type == ReportType.FULL:
                            ...
                            logger.info(f"{display_name}({code}) 使用完整报告格式")
                        else:
                            # 精简报告：使用单股报告格式（默认）
                            report_content = self.notifier.generate_single_stock_report(
                                result
                            )
                            logger.info(f"{display_name}({code}) 使用精简报告格式")

                        if self.notifier.send(report_content, email_stock_codes=[code]):
                            logger.info(f"{display_name}({code}) 单股推送成功")
                        else:
                            logger.warning(f"{display_name}({code}) 单股推送失败")
                    except Exception as e:
                        logger.error(f"{display_name}({code}) 单股推送异常: {e}")

            return result

        except Exception as e:
            # 捕获所有异常，确保单股失败不影响整体
            logger.exception(f"{stock_name}({code}) 处理过程发生未知异常: {e}")
            return None
    ...
    def run(
        ...
        report_type = (
            ReportType.FULL if report_type_str == "full" else ReportType.SIMPLE
        )
        # Issue #128: 从配置读取分析间隔
                    ...
                    if channel == NotificationChannel.FEISHU:
                        ...
                        if stock_email_groups:
                            ...
                            for key, group_results in emails_to_results.items():
                                ...
                                if key is None:
                                    non_wechat_success = (
                                        self.notifier.send_to_email(grp_report)
                                        or non_wechat_success
                                    )
                                else:
                                    non_wechat_success = (
                                        self.notifier.send_to_email(
                                            grp_report, receivers=list(key)
                                        )
                                        or non_wechat_success
                                    )
                        else:
